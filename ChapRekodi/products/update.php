...
$id = $_GET["id"];
$query = "SELECT * FROM mauzo where id = '$id'";
...
<body>
    <form method="post">
        ...
        <input type="text" name="beiyakilakuku"  value = '.$row["beiyakilakuku"].' required><br>
        <input type="text" name="jumlayabei"  value = '.$row["jumlayabei"].' required><br>
...
if(isset($_POST['submit'])){
    $id = $_GET["id"];
    $productName = $_POST['nambayakuku'];
    $productPrice = $_POST['beiyakilakuku'];
    $productDescription = $_POST['jumlayabei'];
    $productImage = $_POST['tarehe'];

    //request and execute the query
    $query = "UPDATE mauzo SET nambayakuku = '$productName', beiyakilakuku ='$productPrice', jumlayabei ='$productDescription', tarehe ='$productImage'  WHERE id = $id;";
    $result2 = mysqli_query($connection,$query);
    
    if($result2){
        echo "<script type='text/javascript'>
                    window.location.href = 'viewsales.php?update=Mabadiliko yamefanyika kikamilifu!';
              </script>";
        $connection->close();
    } else {
        echo "<script type='text/javascript'>
